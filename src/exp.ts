...
export function calculateArNSScoresFlat(records: CachedRecords) {
    ...
    for (const key in records) {
        ...
        if (record.contract.records["@"] && record.contract.records["@"].transactionId !== "" &&
            record.contract.records["@"].transactionId !== "UyC5P5qKPZaltMmmZAWdakhlDXsBF6qmyrbWYFchRTk") {
            scores[owner] += 25;
        }
...
export function calculateArNSScores(records: CachedRecords): Scores {
    const scores: Scores = {};
    ...
    for (const key in records) {
        ...
        // Points for having multiple undernames
        if (Object.keys(record.contract.records).length > 1 && scores[owner].categories.multipleUndernames.awarded === false) {
        ...
        if (record.undernames > 10 && scores[owner].categories.moreThanTenUndernames.awarded === false) {
        ...
        if (record.contract.controllers && scores[owner].categories.controllersAddedded.awarded === false) {
            scores[owner].totalPoints += 10;
